"""ValveDrawTool — 在地图上绘制阀门（作为 Link）"""
...
class ValveDrawTool(QgsMapTool):
    """阀门绘制工具 — 点击高压端→点击低压端"""
    ...
    def __init__(self, iface, layer_manager):
        ...
        self.rubber.setStrokeColor(QColor(243, 156, 18))
    ...
    def activate(self):
        ...
        if self.layer_manager:
            self.layer = self.layer_manager.get_layer("valve")
            self.junction_layer = self.layer_manager.get_layer("junction")
        self.iface.messageBar().pushMessage(
            "aQuaDrip", "点击高压端节点 → 点击低压端节点绘制阀门", level=0, duration=5)
    ...
    def _finish_valve(self, end_point, end_node_id):
        ...
        try:
            ...
            self.canvas.refresh()
            self.iface.messageBar().pushMessage(
                "aQuaDrip", f"阀门 {vid} 已绘制 (图层要素数: {self.layer.featureCount()})", level=0, duration=3)
